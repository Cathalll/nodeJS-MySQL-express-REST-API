...
```js
var post  = {id: 1, nameClassroom: 'Hello MySQL'};
var query = connection.query('INSERT INTO posts SET ?', post, function (error, results, fields) {
  if (error) throw error;
  // Neat!
});
console.log(query.sql); // INSERT INTO posts SET `id` = 1, `nameClassroom` = 'Hello MySQL'
```
...
```js
var query = "SELECT * FROM posts WHERE nameClassroom=" + mysql.escape("Hello MySQL");

console.log(query); // SELECT * FROM posts WHERE nameClassroom='Hello MySQL'
```
...
connection.query("UPDATE posts SET nameClassroom = :nameClassroom", { nameClassroom: "Hello MySQL" });
```
...
```js
connection.query('INSERT INTO posts SET ?', {nameClassroom: 'test'}, function (error, results, fields) {
  if (error) throw error;
...
```js
connection.query('DELETE FROM posts WHERE nameClassroom = "wrong"', function (error, results, fields) {
  if (error) throw error;
...
connection.beginTransaction(function(err) {
  if (err) { throw err; }
  connection.query('INSERT INTO posts SET nameClassroom=?', nameClassroom, function (error, results, fields) {
    if (error) {
